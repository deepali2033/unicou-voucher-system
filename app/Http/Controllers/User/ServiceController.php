...
class ServiceController extends Controller
{
    ...
    {
        // Only current user's services
        $services = Service::where('user_id', auth()->id())
            ->orderBy('sort_order')
            ->orderBy('name')
            ->paginate(15);
        return view('user.services.index', compact('services'))->with('approval_status', $services->pluck('approval_status'));
    }
    ...
    public function update(Request $request, Service $service)
    {
        ...
        // Handle image upload
        if ($request->hasFile('image')) {
            // Debug: Log old image deletion
            if ($service->image) {
                \Log::info('Deleting old image: ' . $service->image);
                Storage::disk('public')->delete($service->image);
            }

            // Debug: Log new image upload
            $validated['image'] = $request->file('image')->store('services', 'public');
            \Log::info('Uploaded new image: ' . $validated['image']);
        ...
        \Log::info('Updated service image in database: ' . ($validated['image'] ?? 'No new image'));
