...
@section('page-actions')
    <div class="d-flex gap-2">
        @if($unreadCount > 0)
            <form action="{{ route('recruiter.notifications.mark-all-read') }}" method="POST" class="d-inline">
                @csrf
        ...
        @endif
        <form action="{{ route('recruiter.notifications.delete-all-read') }}" method="POST" class="d-inline"
        ...
        <div class="card-body p-4 koa-tb-cnt">
            @if($notifications->isEmpty())
                <div class="text-center py-5">
                    ...
                    <p class="text-muted">You'll see notifications here when you create or update services and jobs.</p>
                </div>
            @else
                <!-- Notifications List -->
                                        ...
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div class="flex-grow-1">
                                                @if($notification->action_url)
                                                    <a href="{{ $notification->action_url }}" class="text-decoration-none text-dark">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    @if(!$notification->is_read)
                                                        <li>
                                                            <form action="{{ route('recruiter.notifications.mark-as-read', $notification->id) }}" method="POST">
                                                                @csrf
                                                    ...
                                                    @endif
                                                    <li>
                                                        <form action="{{ route('recruiter.notifications.destroy', $notification->id) }}" method="POST"
