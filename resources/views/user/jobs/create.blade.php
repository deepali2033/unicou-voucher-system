...
<div class="row">
    <div class="col-12">
                ...
                <form action="{{ route('user.jobs.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="mb-3">
                            ...
                            @enderror
                            <div class="mt-3">
                                <img id="image-preview" src="#" alt="Image Preview" style="display: none; width: 100%; max-height: 200px; object-fit: cover;">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-between gap-3">
                        <a href="{{ route('user.jobs.index') }}" class="btn koa-badge-green-outline fw-medium px-4 py-2">
        ...
        {{-- <div class="card mt-4 shadow-sm border-0 koa-tb-card" style="background-color: #f4f6f0;">
            <div class="card-header border-0 koa-card-header">
                <h5 class="card-title">All Job Opportunities</h5>
                <div class="mt-3">
                    <form method="GET" action="{{ route('user.jobs.create') }}" class="row g-3">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-4 mb-4">
                            <div class="card h-100">
                                <img src="{{ $job->image ? asset('storage/' . $job->image) : asset('images/default-job.png') }}"
                                     class="card-img-top" alt="{{ $job->title }}" style="height: 150px; object-fit: cover;">
                                <div class="card-body">
                                    <h6 class="card-title">{{ $job->title }}</h6>
                                    <p class="card-text">{{ $job->short_description }}</p>
                                    <p class="text-muted small">{{ $job->location }}</p>
                                    <a href="{{ route('jobs.show', $job->slug) }}" class="btn btn-sm koa-badge-green">View Details</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
...
document.addEventListener('DOMContentLoaded', function() {
    // Category selection - Send to backend
    ...
    if (categorySelect) {
        categorySelect.addEventListener('change', function() {
            ...
            if (selectedCategory) {
                // Send category to backend via AJAX
                fetch('{{ route("user.jobs.select-category") }}', {
                    method: 'POST',
                    headers: {
